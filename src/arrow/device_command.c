...
int arrow_send_event_ans(property_t hid, cmd_type ev, property_t payload) {
	...
	STD_ROUTINE(_event_ans_init, &edata, NULL, NULL,
			"Arrow Event answer cmd_failed...");
}
...
int ev_DeviceCommand(void *_ev, JsonNode *_parameters) {
  ...
  http_session_set_protocol(current_client(), api_via_mqtt);
	...
	if (!cmd)
		cmd = json_find_member(_parameters, p_const("Command"));
	...
	if (!pay)
		pay = json_find_member(_parameters, p_const("Payload"));
...
//  DBG("ev msg: %s", pay->string_);
	...
	device_command_done: http_session_close_set(current_client(), true);
	RETRY_CR(retry);
	if (_error) {
