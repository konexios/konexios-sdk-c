...
int telemetry_response_data_list_free(telemetry_response_data_list_t *data) {
    ...
    konexios_linked_list_for_each_safe(tmp, data->data, telemetry_data_info_t) {
...
int add_telemetry_data_info(telemetry_response_data_list_t *data,
    ...
    konexios_linked_list_init(info);
    info->deviceHid = strdup(deviceHid);
    info->name = strdup(name);
    info->type = strdup(type);
    info->timestamp = timestamp;
    info->floatValue = flval;
    konexios_linked_list_add_node_last(data->data, telemetry_data_info_t, info);
    return 0;
}
...
int konexios_send_telemetry(konexios_device_t *device, void *d) {
...
int konexios_telemetry_find_by_device_hid(const char *hid,
