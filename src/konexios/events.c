...
#include <konexios/device_startstop.h>
#endif
...
sub_t sub_list[] = {
  { "ServerToGateway_DeviceCommand", ev_DeviceCommand, NULL, konexios_command_handler_free },
...
static int check_sign_1(const char *sign,
                        ...
                        JsonNode *parameters) {
  char signature[65] = {0};
  int err = konexios_event_sign(signature,
                             hid,
                             P_VALUE(name),
                             encrypted,
                             parameters);
...
int konexios_mqtt_has_events(void) {
    ...
    konexios_linked_list_for_each(tmp, __event_queue, mqtt_event_t) {
        ret++;
    }
    MQTT_EVENTS_QUEUE_UNLOCK;
    return ret;
...
int konexios_mqtt_event_proc(void) {
    ...
    konexios_mqtt_api_send(tmp, cmd_received);
#endif
...
mqtt_event_proc_error:
    MQTT_EVENTS_QUEUE_LOCK;
    konexios_linked_list_del_node_first(__event_queue, mqtt_event_t);
    MQTT_EVENTS_QUEUE_UNLOCK;
    DBG("\tdeleted, queue size %d", konexios_mqtt_has_events());

#if(ACN_API_ENABLED)
    // Send /succeeded to API
    konexios_mqtt_api_send(tmp, cmd_succeeded);
#endif
...
int konexios_mqtt_api_send(mqtt_event_t *event, cmd_type status) {
	  ...
	  while( konexios_send_event_ans(event->base.id, status, p_null) < 0 ) {
...
int konexios_mqtt_api_wait(int num) {
    ...
    while ( konexios_mqtt_api_has_events() > api_mqtt_max_capacity ) {
        http_response_t dummy;
        konexios_mqtt_api_event_proc(&dummy);
    }
    return 0;
}
...
int process_http_finish() {
  ...
  if ( ( ret = get_event_base(&api_e->base,
    ...
    konexios_linked_list_add_node_last(__api_event_queue, mqtt_api_event_t, api_e);
error:
    if ( ret < 0 ) {
        http_session_force_http(1);
...
int konexios_mqtt_api_has_events(void) {
    ...
    konexios_linked_list_for_each(tmp, __api_event_queue, mqtt_api_event_t) {
        ret++;
    }
    MQTT_EVENTS_QUEUE_UNLOCK;
    return ret;
...
mqtt_api_error:
    MQTT_EVENTS_QUEUE_LOCK;
    konexios_linked_list_del_node_first(__api_event_queue, mqtt_api_event_t);
    MQTT_EVENTS_QUEUE_UNLOCK;
