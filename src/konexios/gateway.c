...
P_ADD(konexios_gateway_config, host);
P_ADD(konexios_gateway_config, accessKey);
#endif

void konexios_gateway_init(konexios_gateway_t *gate) {
  memset(gate, 0, sizeof(konexios_gateway_t));
}
...
int konexios_gateway_parse(konexios_gateway_t *gate, const char *s) {
  if (!s) return -1;
  DBG("parse this: %s", s);
  int ret = -1;
...
void konexios_gateway_config_init(konexios_gateway_config_t *config) {
  memset(config, 0x00, sizeof(konexios_gateway_config_t));
  config->type = unknown;
}
