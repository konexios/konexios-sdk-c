...
static konexios_mqtt_delivery_callback_t base_event_callbacks = {
    p_static_null,
    process_event_init,
    process_event,
...
static konexios_mqtt_delivery_callback_t http_event_callbacks = {
    p_static_null,
    process_http_init,
    process_http,
    process_http_finish,
...
static int _mqtt_init_common(mqtt_env_t *env) {
  ...
  env->port = konexios_mqtt_host()->port;
  env->init = 0;
  konexios_linked_list_init(env);
  return 0;
}
...
static int _mqtt_deinit_common(mqtt_env_t *env) {
  ...
  konexios_mutex_deinit(env->client.mutex);
#endif
...
mqtt_env_t *get_telemetry_env()
{
  ...
  if ( !tmp ) {
    ...
    konexios_linked_list_add_node_last(__mqtt_channels,
                                    mqtt_env_t,
                                    tmp);
  }
...
// Arrow MQTT logic
int mqtt_telemetry_connect(konexios_gateway_t *gateway,
                           konexios_device_t *device,
                           konexios_gateway_config_t *config) {
...
int mqtt_telemetry_terminate(void) {
  ...
  if ( ! _mqtt_env_is_init(tmp,  MQTT_SUBSCRIBE_INIT) ) {
    konexios_linked_list_del_node(__mqtt_channels, mqtt_env_t, tmp);
    _mqtt_env_free(tmp);
#if !defined(STATIC_MQTT_ENV)
    free(tmp);
#endif
...
int mqtt_publish(konexios_device_t *device, void *d) {
...
void mqtt_disconnect(void) {
  ...
  konexios_linked_list_for_each ( curr, __mqtt_channels, mqtt_env_t ) {
...
void mqtt_terminate(void) {
  mqtt_env_t *curr = NULL;
  konexios_linked_list_for_each_safe ( curr, __mqtt_channels, mqtt_env_t ) {
    if ( curr->init ) {
      _mqtt_env_free(curr);
    }
#if !defined(STATIC_MQTT_ENV)
...
int mqtt_subscribe_connect(konexios_gateway_t *gateway,
                           konexios_device_t *device,
                           konexios_gateway_config_t *config) {
  ...
  if ( konexios_mqtt_client_delivery_message_reg(&tmp->client, &base_event_callbacks) < 0 ) {
      DBG("MQTT handler fail");
  }

#if (ACN_API_ENABLED) //defined(HTTP_VIA_MQTT)
  property_weak_copy(&http_event_callbacks.topic, tmp->s_api_topic);
  if ( konexios_mqtt_client_delivery_message_reg(&tmp->client, &http_event_callbacks) < 0 ) {
...
int mqtt_subscribe(void) {
  ...
  if ( tmp &&
    ...
    int rc = konexios_mqtt_client_subscribe(&tmp->client,
                           QOS2,
    ...
    rc = konexios_mqtt_client_subscribe(&tmp->client,
                               QOS2,
